function [X,Y] = ode4xyu(odefun,tspan,x0,U,varargin)
...
%
% Modified by: Daniel Ferreira Viotti (ITA, São José dos Campos, Brazil)
% Date: 2022-02-21
%   -> The modification of the original ode4xy function enables constant
%      input u during integration step
%
...
try
  [f0,y0] = feval(odefun,tspan(1),x0,U(:,1),varargin{:});
catch
...
for i = 2:N
  ...
  xi = X(:,i-1);
  ui = U(:,i-1);
  [f1,y1] = feval(odefun, ti,        xi,           ui, varargin{:});
  [f2,y2] = feval(odefun, ti+0.5*hi, xi+0.5*hi*f1, ui, varargin{:});
  [f3,y3] = feval(odefun, ti+0.5*hi, xi+0.5*hi*f2, ui, varargin{:});  
  [f4,y4] = feval(odefun, tspan(i),  xi+hi*f3,     ui, varargin{:});
